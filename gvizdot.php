...
print("{".PHP_EOL);
print("graph [fontname = \"helvetica\" tooltip=\"" . htmlspecialchars($course->fullname) . "\" ranksep=\"". ($advMap->nodeseperation*0.5) ."\" nodesep=\"" . ($advMap->nodeseperation*0.25) . "\" splines=\"" . ($advMap->edgestyle)."\" ];".PHP_EOL);
...
foreach ($modinfo->cms as $id => $othercm) {
    ...
    if ($othercm->completion && (empty($cm) || $cm->id != $id) && !$othercm->deletioninprogress && $othercm->visible) {
        ...
        $gvnodeattributes["label"] = "<b>" . htmlspecialchars($othercm->name) . "</b>";
        $gvnodeattributes["tooltip"] = htmlspecialchars($othercm->name);
...
foreach ($nodesWithoutInfo as $node) 
{
    ...
    if(startswith($node, "cm_")) // cm indicates a course module
    {
        ...
        print($node . " [style=\"dotted\" fontcolor=\"slategrey\" label=<<I>" . htmlspecialchars($modinfo->cms[$nodeid]->name) . "</I>> tooltip=\"" . get_string('activity_from_other_section', 'activitymap') . "\"]" . PHP_EOL);
...
if ($advMap->content == "allSectionsGrouped")
{
    ...
    foreach($gvsubgr as $subgraph => $nodeids) 
    {
        ...
        if($secInfo->name)
        {
            ...
            print("  tooltip=<" . htmlspecialchars($secInfo->name) . ">;" . PHP_EOL);
            
            // Print description if we should
            if($secInfo->summary)
            {
                print("  label=< <FONT POINT-SIZE=\"20\"><B><U>" . htmlspecialchars($secInfo->name) . "</U></B></FONT> <BR/><BR/>" . PHP_EOL . convertToGraphvizTextitem($secInfo->summary) . " >" . PHP_EOL);
            }
            else
            {
                print("  label=< <FONT POINT-SIZE=\"20\"><B><U>" . htmlspecialchars($secInfo->name) . "</U></B></FONT> >" . PHP_EOL);
            }
